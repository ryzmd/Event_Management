...
<div class="navbar navbar-inverse">
	<div class="navbar-inner">
    	<a class="brand" href="#">Event Management</a>
   		...
   		<ul class="nav">
      		<li class="active"><a href="<?php echo base_url()?>main/home">Home</a></li>
...
<script type="text/javascript">
      ...
      if(type=='event')
      {
          $.ajax({
            ...
            success:function(arr){
                ...
                $("#get_list").html("<button class='btn btn-primary span4 offset4'>Show participant</button>");
            }
          });
      }
      else
  ...
  $('#get_list').on('click','button',function(){
    $('#group_list').html('');
    $.ajax({
      url:'<?php echo base_url("participant/find_participants") ?>',
      method:'POST',
      data:'event_id='+id,
      success:function (result){
        console.log(result);
        $('#group_list').append('<br>');
        $('#group_list').append('<h3>User who registered for same event:</h3>');
        for(var i=0;i<result.length;i++)
        {
          $('#group_list').append('<h5>'+result[i]["username"]+'</h5>');
        }
